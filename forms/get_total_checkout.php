...
$sql = $db->prepare("SELECT COUNT(business_name) AS total_vendors FROM checkout WHERE checkout_date = ? AND market_id = ? ");
$sql->execute(array($date, $market_id));
$total = $sql->fetch();
$total_vendors = $total['total_vendors'];


$q1 = $db->prepare("SELECT SUM(total_owed) AS paid FROM checkout WHERE checkout_date = ? AND market_id = ? ");
$q1->execute(array($date, $market_id));
$total = $q1->fetch();
        ...
        <?php } if (in_array('ncga', $currency_names)) {?>
              ...
              <?php } if (in_array('end_of_market', $currency_names)) {?>
                  <div class="form-group">
                    ...
                    <input readonly name="ebt_tokens_in" value="$<?= $ebt_tokens_sum?>" class="form-control" >
                  </div>
           ...
           <?php } if (in_array('end_of_market', $currency_names)) {?>
             <div class="form-group">
               <label>EBT Total In</label>
               <input  name="ebt_total_in" value="" placeholder="Enter Total EBT Collect" class="form-control" >
             </div>
             <div class="form-group">
               <label>ATM Out</label>
             ...
             <div class="form-group">
               <label>EBT Out</label>
               <input  name="ebt_out" value="" placeholder="Enter Total EBT Out" class="form-control" >
             </div>
             <div class="form-group">
               <label>EBT Match Out</label>
               <input  name="ebt_match_out" value="" placeholder="Enter Total EBT Match Out" class="form-control" >
             </div>
             <div class="form-group">
